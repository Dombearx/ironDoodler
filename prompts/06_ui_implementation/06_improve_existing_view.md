...
Przejrzyj istniejącą implementację widoku:

<existing_view_implementation>
{{view}} <- zamień na istniejącą implementację widoku
</existing_view_implementation>

Przejrzyj nowe wymagania:

<new_requirements>
Widok listy fiszek musi umożliwić edycję i usuwanie fiszek.
Zarówno przód (front) jak i tył (back) musi być możliwy do edytowania. Gdy użytkownik zakończy edytowanie pola wynik musi zostać zapisany do bazy. Po prawej stronie każdej fiszki musi być przycisk który umożliwia jej usunięcie z bazy. 
</new_requirements>
